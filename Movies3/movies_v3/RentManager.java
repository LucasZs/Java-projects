...
public class RentManager {
	...
	private static ObjectInputStream objectsFromSocket;
	private static ObjectOutputStream objectsToSocket;
	...
	private static void shutDownServer() {
		try {
			objectsToSocket.writeObject(Command.EXIT);
	...
	private static void sendData(List<Product> products) throws IOException {
		objectsToSocket.writeObject(Command.PUT);
		for (Product item: products) {
			objectsToSocket.writeObject(item);
		}
	}
	
	private static List<Product> receiveData() throws ClassCastException, ClassNotFoundException, IOException {
		objectsToSocket.writeObject(Command.GET);
		List<Product> products = (List<Product>)objectsFromSocket.readObject();
		return products;
	}
