/*
Write a method which gets an array of integers (int) as a parameter and reorders the elements the following way (and returns with the ordered array):
First element is an odd number, if there is one.
Every odd number should be followed with an even number, if there is one and every even number should be followed with an odd number, if there is one (eg. [1,4,5,8,...etc.])
